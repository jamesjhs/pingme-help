...
<body data-view="${escapeHtml(view)}">
  <div class="shell">
    ...
    <footer class="footer">
      <div>©PingMe.HELP 2026 | v0.0.1</div>
      <a class="privacy-link" href="/privacy">Privacy Policy</a>
    </footer>
  </div>
</body>
...
function commonFieldAttrs() {
  return 'autocomplete="one-time-code" autocapitalize="none" autocorrect="off" spellcheck="false"';
}

function renderHomePage(siteKey) {
  const attrs = commonFieldAttrs();
  return renderLayout({
    title: 'pingme.help',
    view: 'home',
    ...
    content: `
      ...
      <section class="tabs card" aria-label="Dashboard tabs">
        <div class="tab-row" role="tablist">
          <button class="tab-button is-active" type="button" data-tab-target="update-panel" role="tab" aria-selected="true">Update Status</button>
          <button class="tab-button" type="button" data-tab-target="delete-panel" role="tab" aria-selected="false">Delete Account</button>
        </div>

        <section id="update-panel" class="tab-panel is-active" role="tabpanel">
          <form id="status-form" class="stack-form" novalidate>
            <label>
              <span>Username</span>
              <input name="username" type="text" maxlength="32" required ${attrs}>
            </label>
            <label>
              <span>Password</span>
              <input name="password" type="password" maxlength="128" required ${attrs}>
            </label>
            <label>
              <span>Secret Codeword</span>
              <input name="secretCodeword" type="text" maxlength="64" required ${attrs}>
            </label>
            <label>
              <span>Alert Email <small>(optional)</small></span>
              <input name="alertEmail" type="email" maxlength="254" inputmode="email" ${attrs}>
            </label>
            <label>
              <span>Burn Message <small>(optional, 100 characters)</small></span>
              <textarea name="message" maxlength="100" rows="4" ${attrs}></textarea>
            </label>
        ...
        <section id="delete-panel" class="tab-panel" role="tabpanel" hidden>
          <form id="delete-form" class="stack-form" novalidate>
            ...
            <div class="turnstile-shell js-turnstile"></div>
            <button class="destructive-button" type="submit">Delete Account</button>
          </form>
    ...
    content: `
      <section class="card viewer-card">
        ...
        <form id="viewer-access-form" class="stack-form" novalidate>
          <label>
            <span>Secret Codeword</span>
            <input name="codeword" type="password" maxlength="64" required ${attrs}>
          </label>
          <div class="turnstile-shell js-turnstile"></div>
        ...
        <section id="viewer-result" class="hidden">
          ...
          <button id="reveal-message-button" class="blur-card hidden" type="button">Click to View Once</button>
          <div id="revealed-message" class="card inset-card hidden"></div>
          <button id="acknowledge-button" class="primary-button hidden" type="button">Acknowledge</button>
        </section>
      </section>
    `
  });
      ...
      <section class="card admin-card">
        ...
        <form id="admin-login-form" class="stack-form" novalidate>
          <label>
            ...
            <input name="username" type="text" maxlength="64" required ${attrs}>
          </label>
          <label>
        ...
        <form id="admin-reset-form" class="stack-form hidden" novalidate>
          <label>
            ...
            <input name="newPassword" type="password" maxlength="128" required ${attrs}>
          </label>
          <div class="turnstile-shell js-turnstile"></div>
  ...
  return renderLayout({
    ...
    content: `
      <article class="card prose-card">
        ...
        <p>The encrypted SQLite database stores only the fields needed to run the service: usernames, password hashes, readiness state, a shared secret codeword, an optional burn-after-read message, simplified “Last Updated” and “Last Accessed” text values, whether the burn message was viewed, and an optional alert email if a user chooses to configure one. These values are stored without IP references and without hidden server metadata fields.</p>
        ...
        <p>The “Last Updated” and “Last Accessed” indicators are simplified data values created to inform users and followers when activity occurred. They are stored securely without attached system metadata, IP references, or hidden tracking identifiers.</p>
        ...
        <p>Because pingme.help uses Cloudflare for DDoS protection and domain resolution, network-level transit logs remain subject to Cloudflare’s global edge infrastructure privacy rules. Those network-layer logs are outside the application database and outside the Node.js runtime.</p>
