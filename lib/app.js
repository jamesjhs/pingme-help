...
// Built once at module load — no per-request string allocation
const CSP = [
  "default-src 'self'",
  "base-uri 'self'",
  "form-action 'self'",
...
function sendAsset(response, asset) {
  // Static assets are immutable for the lifetime of the process — allow
  // caching but require revalidation so a deploy gets fresh files promptly.
...
function createServer({ config, store }) {
  ...
  // Pre-render the three fully-static pages once so every GET request serves
  // a pre-built buffer instead of re-running the template on each hit.
  const homePageHtml = renderHomePage(config.turnstileSiteKey);
  const adminPageHtml = renderAdminPage(config.turnstileSiteKey);
